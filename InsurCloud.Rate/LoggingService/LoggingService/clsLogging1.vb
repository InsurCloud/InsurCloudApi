...
Public Class clsLogging1
    Inherits clsLogging

    Protected moPolicy As CorPolicy.clsPolicyHomeOwner

    Public Property Policy() As CorPolicy.clsPolicyHomeOwner
        Get
            Return moPolicy
        End Get
        Set(ByVal value As CorPolicy.clsPolicyHomeOwner)
            moPolicy = value
        End Set
    End Property
    ...
    Public Overrides Function GetInsertSQL() As String
        ...
        sSql = sSql & " VALUES ( @QuoteID , @PolicyID , @FilePath , @StartTS , @EndTS, @UnitNum, @Premium , @Fees , @AgencyID , @UserID , @FirstName , @LastName , @Territory , @Region , @Zip , @PlaceCode , @LossLevel , @HomeAge , @OwnerOccupiedFlag , @ProtectionClass , @Construction , @DwellingAmt , @ContentsAmt , @FireDept , @OtherStructureAmt, @LogXML ) "
        Return ssql
    End Function
    Public Overrides Sub GetInsertParms(ByVal sXML As System.Xml.XmlDocument, ByRef cmd As System.Data.SqlClient.SqlCommand)
        ...
        cmd.Parameters.Add("@Fees", SqlDbType.Decimal).Value = IIf(Me.Policy.TotalFees = Nothing, DBNull.Value, Me.Policy.TotalFees)
